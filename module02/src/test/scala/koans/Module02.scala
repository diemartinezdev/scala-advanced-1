...
class Module02 extends KoanSuite with Matchers with SeveredStackTraces {
  ...
  // of the the put method. This Car is an immutable object, so add returns a new Car, potentially of
  // a more general type:
  ...
  test("A Dock should be able to handle all cargo of a more specific type than it is defined on") {
    // in this example, Ship's type param is invariant, the point is to *not* change the variance of Ship,
    // but instead to change the variance of the Dock definition below it!
